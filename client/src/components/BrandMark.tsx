...
export default function BrandMark({ inverted = false }: { inverted?: boolean }) {
  ...
  return (
    <span aria-hidden="true" className={`relative grid h-8 w-8 shrink-0 place-items-center rounded-full border-2 ${ring}`}>
      <span className={`absolute inset-[3px] rounded-full border ${inner}`} />
      <span className={`h-1.5 w-1.5 rounded-full ${dot}`} />
      <span className={`absolute -right-1 top-1/2 h-px w-2 -translate-y-1/2 ${dot}`} />
    </span>
  );
}
